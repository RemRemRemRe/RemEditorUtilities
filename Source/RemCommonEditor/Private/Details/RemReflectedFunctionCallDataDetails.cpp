    ...
    FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
    ...
    static auto RestrictReason = NSLOCTEXT("RemReflectedFunctionCallData", "PassingClassFilter",
        "Passing meta data of class filter to FunctionOwnerClass");
...
void FRemReflectedFunctionCallDataDetails::OnFunctionNameChanged(
    const FPropertyChangedEvent& PropertyChangedEvent) const
{
    RemCheckVariable(FunctionCallDataPropertyHandle, return;);

    auto* FunctionCallData{
        Rem::Editor::GetStructPtr<FRemReflectedFunctionCallData>(FunctionCallDataPropertyHandle.ToSharedRef())
    };
